...
('Mk.7 "Aquila"', 2, 14, 1500, 1, 6, null),
('Mk.8 "Errant"', 2, 14, 1700, 1, 7, null),
('Mk.7 with Repair elemnt', 2, 15, 1400, 1, 8, null),
('Eldar Soul Bone', 4, 10, 900, 2, 5, null),
('Mk.4 "Maximus"', 2, 14, 1700, 2, 6, null)
;
...
CREATE OR REPLACE FUNCTION public.armourracetrigger()
    RETURNS trigger
    LANGUAGE 'plpgsql'
    COST 100
    VOLATILE NOT LEAKPROOF
...
ALTER FUNCTION public.armourracetrigger()
    OWNER TO postgres;
...
-- FUNCTION: public.checkeffectsid()

-- DROP FUNCTION IF EXISTS public.checkeffectsid();

CREATE OR REPLACE FUNCTION public.checkeffectsid()
    RETURNS trigger
    LANGUAGE 'plpgsql'
    COST 100
    VOLATILE NOT LEAKPROOF
...
ALTER FUNCTION public.checkeffectsid()
    OWNER TO postgres;
...
-- FUNCTION: public.checkweapons()

-- DROP FUNCTION IF EXISTS public.checkweapons();

CREATE OR REPLACE FUNCTION public.checkweapons()
    RETURNS trigger
    LANGUAGE 'plpgsql'
    COST 100
    VOLATILE NOT LEAKPROOF
